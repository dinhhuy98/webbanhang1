<?php 
	...
	$pagination=loadClass("pagination");
  	...
  	if(isset($_GET['cat'])){
		$numberPage= ceil($category->countItem($_GET['cat'])/$numberItem);
		$data=$category->selectItem($_GET['cat']);
  	}
	else if(isset($_GET['tag'])){
		$numberPage= ceil($tag->countItem($_GET['tag'])/$numberItem);
		$data=$tag->selectItem($_GET['tag']);
	}
  $start=0;
